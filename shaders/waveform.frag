...
/* helpers */
ivec2 tileXY(int i, int w) { return ivec2(i - (i / w) * w, i / w); }

float fetchSample(int idx) {
  idx = clamp(idx, 0, u_sampleCount - 1);
  ivec2 c = tileXY(idx, u_sampW);
  return Texel(SampTex, vec2((float(c.x) + 0.5) / float(u_sampW),
...
vec4 fetchWindow(int idx) {
  ...
  return Texel(ColTex, vec2((float(c.x) + 0.5) / float(u_winW),
...
vec4 effect(vec4 v, Image dummy, vec2 uv, vec2 pc) {
  ...
  if (u_samplesPerPixel <= 1.0) {
    ...
    vec3 col = mix(fetchWindow(w0).rgb, fetchWindow(w1).rgb, fw);
